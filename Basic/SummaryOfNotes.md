...
拾柒.线程进程和协程(017ProcessAndThreadAndCoroutine)
    ...
    进程
        进程状态
            ...
            执行状态
            等待状态/阻塞状态
        进程常用属性和方法(myprocess_function_attributes.py)
            | 属性         | 含义                                  |
            | ---------- | ----------------------------------- |
            | `pid`      | 当前进程的系统进程 ID                        |
            | `name`     | 进程名称,默认格式 "Process-N"               |
            | `daemon`   | 是否为守护进程（True/False）                 |
            | `exitcode` | 进程退出状态,None 表示还在运行,0 正常退出,负数代表被信号终止 |
            | `authkey`  | 进程之间的认证 key,通常用于底层 IPC              |
            | 方法              | 作用                      |
            | --------------- | ----------------------- |
            | `start()`       | 启动进程                    |
            | `run()`         | 执行指定 target 函数（一般不直接调用） |
            | `join(timeout)` | 阻塞主进程等待子进程完成            |
            | `terminate()`   | 强制终止进程                  |
            | `kill()`        | 直接杀死进程（3.7+）            |
            | `is_alive()`    | 判断进程是否还存活               |
...
拾捌.正则表达式(018Regular)
    ...
    常用flags
        ...
        | `re.M` / `re.MULTILINE`  | 多行模式，`^`、`$` 匹配每行 |
        | `re.S` / `re.DOTALL`     | `.` 匹配包括换行        |
        | `re.X` / `re.VERBOSE`    | 允许正则换行和注释，便于阅读    |
    原生字符串
        用前缀r表示, 原生字符串取消转义
        s = r"Hello\nWorld"
        输出结果不会解析 \n, 而是原样显示: Hello\nWorld
...
拾玖.内置模块(019BuiltInModule)
    
    os模块                        操作系统、环境变量、进程等
    sys模块                       Python 运行环境、解释器参数、路径等
    time模块                      时间戳、睡眠、简单时间处理
        | 类型                    | 示例                      | 描述                                   |
        | --------------------- | ----------------------- | ------------------------------------ |
        | **时间戳（timestamp）**    | `1730863122.123`        | 从 1970-01-01 00:00:00 UTC 到现在的秒数（浮点） |
        | **时间元组（struct_time）** | `time.localtime()`      | 9 个字段：年、月、日、时、分、秒、星期、年度第几天、夏令时       |
        | **格式化字符串（str）**       | `"2025-11-06 22:30:12"` | yyyy-mm-dd hh:mm:ss                  |
        时间戳 ↔ 时间元组
            | 方法                   | 转换方向              |
            | -------------------- | ----------------- |
            | `time.localtime(ts)` | 时间戳 → **本地**时间元组  |
            | `time.gmtime(ts)`    | 时间戳 → **UTC**时间元组 |
            | `time.mktime(tuple)` | 时间元组 → 时间戳        |
        时间元组 ↔ 格式化字符串
            | 方法                          | 转换方向            |
            | --------------------------- | --------------- |
            | `time.strftime(fmt, tuple)` | 时间元组 → 字符串（格式化） |
            | `time.strptime(str, fmt)`   | 字符串 → 时间元组      |
    logging模块                   日志记录
        日志级别
            | 级别       | 说明            | 常用方法                 |
            | -------- | ------------- | -------------------- |
            | DEBUG    | 调试信息，开发阶段最详细  | `logging.debug()`    |
            | INFO     | 程序运行信息，正常输出   | `logging.info()`     |
            | WARNING  | 警告信息，不影响程序运行  | `logging.warning()`  |
            | ERROR    | 错误信息，程序可以处理   | `logging.error()`    |
            | CRITICAL | 严重错误，程序可能无法运行 | `logging.critical()` |
        日志输出到文件
            logging.basicConfig(
                level=logging.INFO,
